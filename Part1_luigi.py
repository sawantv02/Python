                    ...
                    with open(newFile, 'a', encoding='utf-8', newline="") as file:
                        for f in glob.glob(str(os.getcwd()) + '\\' + newFile):
                            if writeHeader2 is True:
                                datadf.to_csv(self.output().path , mode='w', header=True, index=False, skipinitialspace=True)
                                writeHeader2 = False
                            else:
                                datadf.to_csv(self.output().path, mode='a', header=False, index=False, skipinitialspace=True)
...
class handleMissingData(luigi.Task):
    ...
    def run(self):
        loan_df = pd.read_csv(getData(loginemail=self.loginemail,loginpassword=self.loginpassword).output().path,low_memory=False,encoding='ISO-8859-1',skipinitialspace=True)
        ...
        loan_df.to_csv(self.output().path, header=True, index=False, skipinitialspace=True)
...
class processData(luigi.Task):
    ...
    def run(self):
        loanfreature_df = pd.read_csv(handleMissingData(loginemail=self.loginemail,loginpassword=self.loginpassword).output().path,low_memory=False,encoding='ISO-8859-1',skipinitialspace=True)
        ...
        loanfreature_df.to_csv(self.output().path, header=True, index=False, skipinitialspace=True)
...
class featureSelection(luigi.Task):
    ...
    def run(self):
        loanfreature_df = pd.read_csv(processData(loginemail=self.loginemail, loginpassword=self.loginpassword).output().path,low_memory=False,encoding='ISO-8859-1',skipinitialspace=True)
