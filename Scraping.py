...
class mergeData(luigi.Task):
    ...
    def run(self):
        ...
        demo_durg_df = pd.merge(drug, demo, on=('primaryid', 'caseid'), how='inner')
        ...
        demodrugreactout_df.to_csv(self.output().path, header=True, index=False, skipinitialspace=True);
